...
use sbx::cli::{
    ClaudeCmd, Cli, Cmd, EnvCmd, NetCmd, PortCmd, ProfileCmd, ServiceCmd, SshCmd, StartCmd,
    TailscaleCmd, VpnCmd,
};
use sbx::commands;
use sbx::env_file;
...
fn main() {
    ...
    match cli.command {
        ...
        Some(Cmd::Env { action }) => match action.unwrap_or(EnvCmd::List) {
        ...
        Some(Cmd::Start { action, rest }) => match action {
        ...
        None => match cli.flavor {
            None => std::process::exit(commands::shell::from_project(&cwd())),
...
fn parse_env_set(spec: &str, value: Option<&str>) -> (String, String) {
    ...
    match value {
        ...
        None => die("usage: sbx env set KEY=VALUE  (or: sbx env set KEY VALUE)"),
    }
}
